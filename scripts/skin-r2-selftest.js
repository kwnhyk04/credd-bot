...
async function main() {
  const classBattleEntries = buildCosmeticEntries().filter(
    (entry) => entry.cosmetic_key && entry.cosmetic_key.startsWith('class_battle_')
  );
  assert.equal(classBattleEntries.length, 5, 'all class battle defaults must be seeded');
  assert(
    classBattleEntries.every((entry) => /^classes\/battle_base\/battle_[a-z]+\.png$/.test(entry.render_filename)),
    'class battle defaults must use the battle_<class>.png asset names'
  );

  const devId = DEV_ACCOUNT_IDS[0];
  assert.ok(devId, 'profile-label regression requires a configured developer account');
  const founderId = 'skin-r2-real-founder';
  const labelDb = {
    async query(sql, params = []) {
      if (sql.includes('FROM supporters')) {
        return {
          rows: params[0] === founderId
            ? [{
              discord_id: founderId,
              tier: 'eternal',
              status: 'active',
      ...
      if (sql.includes('FROM equipped_skins')) return { rows: [] };
      throw new Error(`Unexpected profile-label query: ${sql}`);
    },
  };
  assert.deepEqual(
    await resolveProfileLabel(labelDb, devId),
    { hasTopLabel: false, word: null },
    'developer accounts without a real supporter record must not render Founder 000'
  );
  assert.deepEqual(
    await resolveProfileLabel(labelDb, founderId),
    { hasTopLabel: true, word: 'Founder 007' },
    'real founders must retain their zero-padded founder number'
  );
