...
CREATE TABLE users_bag (
    ...
    custom_avatar_token INTEGER NOT NULL DEFAULT 0 CHECK (custom_avatar_token >= 0),
    custom_deity_token  INTEGER NOT NULL DEFAULT 0 CHECK (custom_deity_token >= 0)
);
...
CREATE INDEX idx_tickets_status_type_created ON tickets (status, type, created_at);
...
CREATE TABLE supporter_item_grants (
    ...
    item_key     VARCHAR(32) NOT NULL CHECK (item_key IN ('custom_deity_token')),
...
ALTER TABLE supporter_item_grants ENABLE ROW LEVEL SECURITY;
...
CREATE TABLE user_character (
    ...
    active_preset_slot        SMALLINT    NOT NULL DEFAULT 1 CHECK (active_preset_slot IN (1,2)),
    highest_raid_streak        INTEGER     NOT NULL DEFAULT 0 CHECK (highest_raid_streak >= 0),
    highest_rank_streak        INTEGER     NOT NULL DEFAULT 0 CHECK (highest_rank_streak >= 0),
...
-- 8b. user_presets (active deity/equipment source; legacy columns remain until
--     scripts/migrations/20260803_05_drop_legacy_loadout.sql is deployed)
CREATE TABLE user_presets (
    id                       BIGSERIAL PRIMARY KEY,
    discord_id               VARCHAR(20) NOT NULL REFERENCES users (discord_id) ON DELETE CASCADE,
    slot                     SMALLINT NOT NULL CHECK (slot IN (1,2)),
    name                     VARCHAR(32),
    equipped_deity_1_id      INTEGER REFERENCES user_deities (user_deity_id) ON DELETE SET NULL,
    equipped_deity_2_id      INTEGER REFERENCES user_deities (user_deity_id) ON DELETE SET NULL,
    equipped_deity_3_id      INTEGER REFERENCES user_deities (user_deity_id) ON DELETE SET NULL,
    equipped_echo_deity_id   INTEGER REFERENCES user_deities (user_deity_id) ON DELETE SET NULL,
    equipped_armor_id        VARCHAR(8) REFERENCES user_armors (armor_id) ON DELETE SET NULL,
    equipped_weapon_id       VARCHAR(8) REFERENCES user_weapons (weapon_id) ON DELETE SET NULL,
    updated_at               TIMESTAMPTZ NOT NULL DEFAULT NOW(),
...
CREATE INDEX idx_user_presets_discord_id ON user_presets (discord_id);
...
CREATE TABLE boss_state (
    ...
    expires_at  TIMESTAMPTZ NOT NULL,                            -- terminal timestamp or scheduler cooldown anchor
    status      VARCHAR(10) NOT NULL DEFAULT 'active' CHECK (status IN ('active','dead','escaped')),
    spawn_source VARCHAR(10) NOT NULL DEFAULT 'natural',
...
CREATE UNIQUE INDEX boss_spawn_queue_one_live_per_guild
  ON boss_spawn_queue (guild_id) WHERE status IN ('pending','spawning');
ALTER TABLE boss_spawn_queue ENABLE ROW LEVEL SECURITY;
