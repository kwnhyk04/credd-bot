 ...
 * overhaul. Clears every preset loadout slot for every character, then deletes
 * ALL user_weapons + user_armors rows. Nothing else (currency/chests/level/
 * quests/runes) is touched. Run ONCE after the preset migration:
 *   node scripts/v5_wipe_test_gear.js
 * Idempotent (re-running just reports 0 removed).
 */

const pool = require('../src/db/pool');
const { clearPresets } = require('../src/engine/loadout');
...
(async () => {
  ...
  try {
    ...
    for (const row of characters.rows) {
      await clearPresets(client, row.discord_id);
    }
