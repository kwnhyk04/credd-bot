...
VALUES
    ...
    ('weapon', 'Gungnir', 'gungnir', 'Gains +10% ATK at the start of each turn, stacking up to +50%; all stacks reset after battle. Each attack ignores 30% of enemy DEF and has a 20% chance to use 60% total DEF penetration for that attack.'),
    ('weapon', 'Thunderbolt of Zeus', 'thunderbolt_of_zeus', 'Gains +10% ATK at the start of each turn, stacking up to +50%; all stacks reset after battle. Each critical attack deals +100% bonus ATK and applies Paralyze for 1 turn.'),
    ('weapon', 'Katana', 'katana', 'Each attack deals 30% additional damage (×1.30 on a normal hit; ×2.30 on a critical hit).'),
    ('weapon', 'Kiri', 'kiri', 'Each attack increases damage by 20%, stacking up to +120%. Each attack has a 25% chance to strike twice as two separate hits.'),
