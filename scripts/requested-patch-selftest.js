...
const { groupSummonResults, RARITY_SYMBOLS } = require('../src/engine/renderSummon');
const { buildListPage } = require('../src/commands/rpg/deity');
const { emoji, deityTierEmoji, gearTierEmoji } = require('../src/utils/emojis');
const {
  DEITY_UPDATES, WEAPON_UPDATES, MOB_UPDATES, validateDefinitions,
...
async function main() {
  ...
  assert(bleedTicks.includes(20));
  assert(Math.max(...bleedTicks) <= 20);
  ...
  assert.equal(bathala.playerDefMult, 0.4);
  ...
  assert(compareWeapon.startsWith(`\`weapon-one\` ${gearTierEmoji('Mythic')} `));
  ...
  assert(compareArmor.startsWith(`\`armor-one\` ${gearTierEmoji('Supreme')} `));
  ...
  assert(!newSummonLine.includes('Undying'));
  assert(!newSummonLine.includes('Essence'));
  assert(!newSummonLine.includes('+0'));

  const expectedTierIcons = {
    Remnant: emoji('epic_icon'),
    Awakened: emoji('mythical_icon'),
    Undying: emoji('legendary_icon'),
    Primordial: emoji('supreme_icon'),
  };
  assert.deepEqual(RARITY_SYMBOLS, expectedTierIcons);
  assert.equal(deityTierEmoji('Mythic'), expectedTierIcons.Awakened);
  assert(duplicateSummonLine.includes(expectedTierIcons.Awakened));
  assert(newSummonLine.includes(expectedTierIcons.Undying));

  const deityCollection = await buildListPage({
    user: { id: '123' },
    deities: [
      { name: 'Apolaki', tier: 'Legendary', owned: true },
      { name: 'Mayari', tier: 'Mythic', owned: true },
      { name: 'Bathala', tier: 'Supreme', owned: false },
      { name: 'Amihan', tier: 'Epic', owned: false },
    ],
    mythology: 'PH',
    page: 0,
    totalPages: 1,
  ...
  const deityCollectionJson = JSON.stringify(
    deityCollection.components.map((component) => component.toJSON())
  );
  for (const icon of Object.values(expectedTierIcons)) {
    assert(deityCollectionJson.includes(icon), `deity collection missing tier icon ${icon}`);
  }
