...
<!-- Historical v4.2 baseline. It is not the live source of truth; use CREDD_Current_Masterfile.md for current values and the 2026-07-30 passive/balance corrections. -->
...
| Loki | 1,000 | 430 | 215 | Trickster — high ATK | Illusory Double — 25% chance each turn to evade an attack and counter for 100% ATK |
...
- **Database-authored stats:** Greater and normal bosses use the same `mob_roster` formula (`base_* + *_per_level × boss_level`). Normal stats are unmultiplied. Greater ATK/DEF/CRIT are also unmultiplied; only max HP is adjusted by the fixed chest outcome (×1.5 for 2× Treasure, ×2 for 1× Golden).
...
| Chest | 1× Boss Treasure Chest | **80% → 2× Boss Treasure Chest + ×1.5 HP / 20% → 1× Boss Golden Chest + ×2 HP** (rolled once at spawn, fixed for that fight, and recovered from persisted HP after restart) |

- **Announcement:** Greater spawns get a distinct header (`☠️ GREATER BOSS — A world-ender awakes…`); the pool bar shows the database-authored HP. Announcement and payout both read the same `GREATER_BOSSES` set, so they never disagree.
...
| `loki_illusory_double` | Loki (Leg) | 25%/turn: evade an attack and counter for 100% ATK (rider — advances no counter) |
