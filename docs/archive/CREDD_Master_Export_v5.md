...
<!-- Historical v5 overlay. It is not the live source of truth; use CREDD_Current_Masterfile.md for current values and the 2026-07-30 passive/balance corrections. -->
...
Every boss first resolves its database-authored base and per-level stats directly. Only Greater max HP is then chest-adjusted: ×1.5 for 2× Boss Treasure Chest or ×2 for 1× Boss Golden Chest. ATK, DEF, and CRIT are never multiplied.
